...
def latex_to_unicode(text: str) -> str:
    ...
    try:
        ...
        math_symbols = {
            r'\times': '×', r'\cdot': '⋅', r'\approx': '≈', r'\leq': '≤', r'\geq': '≥', r'\neq': '≠',
            r'\pm': '±', r'\mp': '∓', r'\to': '→', r'\leftarrow': '←', r'\Rightarrow': '⇒', r'\Leftarrow': '⇐',
            r'\leftrightarrow': '↔', r'\infty': '∞', r'\partial': '∂', r'\aleph': 'ℵ', r'\hbar': 'ℏ',
            r'\%': '%'
        }
        ...
        math_functions = {
            r'\sqrt': '√', r'\sum': '∑', r'\int': '∫', r'\prod': '∏', r'\lim': 'lim', r'\ln': 'ln',
            r'\sin': 'sin', r'\cos': 'cos', r'\tan': 'tan', r'\log': 'log', r'\exp': 'exp'
        }
        ...
        superscript_map = {
            ...
            '5': '⁵', '6': '⁶', '7': '⁷', '8': '⁸', '9': '⁹',
            '+': '⁺', '-': '⁻', '=': '⁼', '(': '⁽', ')': '⁾', 'n': 'ⁿ', 'i': 'ⁱ',
            't': 'ᵗ', 'm': 'ᵐ', 'r': 'ʳ', 's': 'ˢ', 'u': 'ᵘ', 'v': 'ᵛ', 'j': 'ʲ', 'd': 'ᵈ', 'g': 'ᵍ',
            'a': 'ᵃ', 'b': 'ᵇ', 'c': 'ᶜ', 'f': 'ᶠ', 'k': 'ᵏ', 'l': 'ˡ', 'o': 'ᵒ', 'p': 'ᵖ', 'q': 'ᑫ',
            'w': 'ʷ', 'x': 'ˣ', 'y': 'ʸ', 'z': 'ᶻ'
        }
...
async def process_user_message(
    ...
    try:
        ...
        await bot.delete_message(chat_id=chat_id, message_id=first_message.message_id)
        ...
        logger.error(f"Ошибка при работе агента для пользователя {user_id}: {e}")
        await bot.send_message(chat_id=chat_id, text=MESSAGES_ERROR["limit_token"]["en"])
    except RateLimitError as e:
        logger.error(f"Ошибка большого количества запросов за раз: {e}")
        await bot.send_message(chat_id=chat_id, text=MESSAGES_ERROR["many_requests"]["en"])
    except Exception as e:
