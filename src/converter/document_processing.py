...
def extract_text_from_txt(file_path: str) -> str:
    ...
    try:
        ...
        raise UnicodeDecodeError(f"Ошибка декодирования текста в файле {file_path}.")
    except IOError as e:
        logger.error(f"Ошибка ввода/вывода при чтении TXT файла {file_path}: {e}")
        raise IOError(f"Не удалось прочитать файл {file_path}.")
    except Exception as e:
        logger.error(f"Неизвестная ошибка при обработке TXT файла {file_path}: {e}")
...
def extract_text_from_odt(file_path: str) -> str:
    ...
    try:
        doc = load(file_path)
        texts: List[str] = [
            text.firstChild.data for text in doc.getElementsByType(P)
        ...
        raise ValueError(f"Файл {file_path} поврежден или не является корректным ODT файлом.")
    except IOError as e:
        logger.error(f"Ошибка ввода/вывода при чтении ODT файла {file_path}: {e}")
        raise IOError(f"Не удалось прочитать файл {file_path}.")
    except Exception as e:
        logger.error(f"Неизвестная ошибка при обработке ODT файла {file_path}: {e}")
        raise Exception(f"Произошла ошибка при извлечении текста из файла {file_path}.")
...
def extract_text_from_presentation(file_path: str) -> Optional[str]:
    ...
    try:
        ...
        raise ValueError(f"Файл {file_path} пустой или не содержит текста.") from e
    except IOError as e:
...
def extract_text_from_zip(file_path: str) -> Optional[str]:
    ...
    try:
            ...
            with zipfile.ZipFile(file_path, 'r') as archive:
                ...
                for file in archive.namelist():
                    ...
                    if mime_type in text_extraction_from_a_document:
                        ...
                        logger.error(f"Файл в ZIP имеет неподдерживаемый формат: {file} (MIME-тип: {mime_type})")

        if not extracted_text:
            raise ValueError(f"Архив {file_path} не содержит файлов с поддерживаемым текстовым форматом.")
        ...
        raise ValueError(f"Ошибка обработки содержимого файла {file_path}.") from e
    except IOError as e:
        logger.error(f"Ошибка ввода/вывода при чтении ZIP-файла: {file_path}")
        raise IOError(f"Ошибка ввода/вывода при чтении {file_path}.") from e
    except Exception as e:
        logger.error(f"Неизвестная ошибка при обработке ZIP-файла: {file_path}")
        raise Exception(f"Произошла ошибка при обработке файла {file_path}.") from e
    ...
    try:
        ...
        with py7zr.SevenZipFile(file_path, mode='r') as archive:
            if not archive.getnames():
                raise ValueError(f"7Z-архив '{file_path}' пуст или некорректен.")
            ...
            for root, _, files in os.walk("tmp"):
                for file in files:
                    ...
                    if mime_type in text_extraction_from_a_document:
                        text_extraction_function = text_extraction_from_a_document[mime_type]
                        with open(file_path_extracted, 'r', encoding='utf-8', errors='ignore') as f:
                        ...
                        logger.error(f"Неподдерживаемый формат файла в 7Z архиве: {file} (MIME-тип: {mime_type})")
        ...
        raise py7zr.SevenZipFileError(f"Файл '{file_path}' не является корректным 7Z-архивом.") from e
    except IOError as e:
        logger.error(f"Ошибка ввода-вывода при обработке 7Z-архива: {e}")
        raise IOError(f"Ошибка ввода-вывода при обработке файла '{file_path}'.") from e
    except Exception as e:
        logger.error(f"Произошла ошибка при обработке 7Z-архива: {e}")
        raise Exception(f"Ошибка при обработке 7Z-архива '{file_path}': {str(e)}") from e
    ...
    try:
        ...
        with tarfile.open(file_path, 'r') as archive:
            if not archive.getmembers():
                raise ValueError(f"TAR-архив '{file_path}' пуст или некорректен.")

            for member in archive.getmembers():
                if member.isfile():
                    mime_type = mimetypes.guess_type(member.name)[0] or 'application/octet-stream'
                        ...
                        with archive.extractfile(member) as f:
                            if f is not None:
                                ...
                                if text_content:
                                    ...
                                    logger.error(f"Не удалось извлечь текст из файла: {member.name}")
                    else:
                        logger.error(f"TAR-архив содержит неподдерживаемый формат: {member.name} (MIME-тип: {mime_type})")

        if not extracted_text:
            raise ValueError(f"TAR-архив '{file_path}' не содержит поддерживаемых файлов.")
        ...
        raise tarfile.ReadError(f"Файл '{file_path}' не является корректным TAR-архивом.") from e
    except IOError as e:
        logger.error(f"Ошибка ввода-вывода при обработке TAR-архива: {e}")
        raise IOError(f"Ошибка ввода-вывода при обработке файла '{file_path}'.") from e
    except Exception as e:
        logger.error(f"Произошла ошибка при обработке TAR-архива: {e}")
        raise Exception(f"Ошибка при обработке TAR-архива '{file_path}': {str(e)}") from e
...
def extract_text_from_mhtml(file_path: str) -> Optional[str]:
    ...
    try:
                ...
                if content_type in ['text/plain', 'text/html']:
                    ...
                    if payload:
                        ...
                        try:
                            ...
                            logger.error(f"Ошибка декодирования содержимого файла {file_path}: {decode_error}")
        ...
        if extracted_text:
            ...
            raise ValueError(f"MHTML-файл '{file_path}' не содержит текстового содержимого.")
        ...
        raise UnicodeDecodeError(f"Не удалось декодировать содержимое файла '{file_path}'.") from e
    except ValueError as e:
        logger.error(f"Файл пуст или некорректный: {e}")
        raise ValueError(f"MHTML-файл '{file_path}' не содержит текстового содержимого.") from e
    except Exception as e:
        logger.error(f"Ошибка при обработке MHTML-файла: {e}")
        raise Exception(f"Произошла ошибка при обработке файла '{file_path}': {str(e)}") from e
...
def extract_text_from_markdown(file_path: str) -> Optional[str]:
    ...
    try:
        ...
        raise Exception(f"Ошибка при обработке файла '{file_path}': {str(e)}") from e
