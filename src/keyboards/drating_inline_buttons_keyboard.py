...
def drating_inline_buttons_keyboard(response_id: int) -> InlineKeyboardMarkup:
    ...
    try:
        if not isinstance(response_id, int) or response_id < 0:
            ...
            raise ValueError(
                f"Некорректное значение response_id: {response_id}"
            )

        buttons = [
            [
                InlineKeyboardButton(
                    text=emoji, callback_data=f"rate_{emoji}_{response_id}"
                )
                for emoji in ["👎", "😐", "👍"]
            ]
        ]
        ...
        logger.info(
            f"Инлайн-клавиатура для оценки создана для response_id: {response_id}"
        )
        return keyboard
        ...
        logger.error(
            f"Неизвестная ошибка при создании инлайн-клавиатуры для response_id {response_id}: {error}"
        )
