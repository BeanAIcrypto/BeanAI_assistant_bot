...
def import_users_and_associated_tables():
    try:
        ...
        with get_db_connection() as conn:
            ...
            conn.commit()
            logger.info("Пользователи и связанные таблицы успешно импортированы.")
    except Exception as error:
        logger.error(f"Ошибка при импорте пользователей и связанных таблиц: {error}")
        raise
...
def import_user_history():
    try:
        ...
        if not os.path.exists(history_file):
            logger.warning(f"Файл {history_file} не найден. Пропускаем импорт.")
            return
